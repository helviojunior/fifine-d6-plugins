/**
 * Claude Usage — StreamDock plugin (Fifine AmpliGame D6 & compatible).
 *
 * Shows real-time Claude rate-limit utilization (5h + 7d windows) on an LCD key
 * as a PNG. Reads the Claude Code OAuth token from the macOS Keychain and makes
 * a 1-token API call every 60s to read the anthropic-ratelimit-unified-* headers.
 *
 * Note: the first poll may trigger a macOS Keychain permission prompt for the
 * StreamDock app to read the "Claude Code-credentials" item — click Allow.
 */
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          try {
            ...
            reject(new Error(`API parse error: ${data.slice(0, 120)}`));
          }
        });
      }
    );
...
function statusColor(s) {
  return { idle: "#666666", low: "#4CAF50", moderate: "#FFA726", high: "#FF7043", critical: "#E57373", limited: "#F44336" }[s];
}
function statusLabel(s) {
  return { idle: "IDLE", low: "LOW", moderate: "MED", high: "HIGH", critical: "CRIT", limited: "LIMIT" }[s];
}

// blend a hex color with the dark bg to fake a translucent badge
function dim(hex, amt = 0.22) {
  const bg = [26, 26, 46];
  const c = [parseInt(hex.slice(1, 3), 16), parseInt(hex.slice(3, 5), 16), parseInt(hex.slice(5, 7), 16)];
  const m = c.map((v, i) => Math.round(v * amt + bg[i] * (1 - amt)));
...
async function renderImage(info) {
  ...
  const resetIn = formatDuration(info.reset5h * 1000 - now);
  ...
  // Bottom row: resets clock / in / claim
...
async function renderError(msg) {
  ...
  rect(ctx, 13, 11, 9, 9, "#D4A574");
  text(ctx, "Claude", 28, 20, 8, "#dddddd", "left", "DeckBold");
  const m = msg.length > 18 ? msg.slice(0, 18) + "…" : msg;
  text(ctx, m, 72, 75, 8, "#E57373", "center");
...
const instances = new Map(); // context -> { timer, lastInfo }
...
async function update(context) {
  ...
  try {
    ...
    if (state.lastInfo) {
      sd.setImage(context, await renderImage(state.lastInfo));
    } else {
      sd.setImage(context, await renderError(err.message));
    }
    sd.setTitle(context, "");
  }
}
...
sd.on("willAppear", (msg) => {
  ...
  instances.set(context, { timer: null, lastInfo: null });
  update(context);
  instances.get(context).timer = setInterval(() => update(context), POLL_MS);
});
...
// Only connect when launched directly by the host (allows render unit tests).
if (require.main === module) sd.connect();

module.exports = { renderImage, renderError };
